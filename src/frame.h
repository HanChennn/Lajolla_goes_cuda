...
struct Frame {
    ...
    __host__ __device__ inline const Vector3& operator[](int i) const {
        return *(&x + i);
    }
    Vector3 x, y, n;
};
...
__host__ __device__ inline Frame operator-(const Frame &frame) {
    return Frame(-frame.x, -frame.y, -frame.n);
}

/// Project a vector to a frame's local coordinates.
__host__ __device__ inline Vector3 to_local(const Frame &frame, const Vector3 &v) {
...
__host__ __device__ inline Vector3 to_world(const Frame &frame, const Vector3 &v) {
