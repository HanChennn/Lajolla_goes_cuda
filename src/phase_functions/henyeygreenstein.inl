...
__host__ __device__ Spectrum eval_op::operator()(const HenyeyGreenstein &p) const {
    return make_const_spectrum(c_INVFOURPI *
        (1 - p.g * p.g) / 
            (pow((1 + p.g * p.g + 2 * p.g * dot(dir_in, dir_out)), Real(3)/Real(2))));
}

__host__ __device__ std::optional<Vector3> sample_phase_function_op::operator()(const HenyeyGreenstein &p) const {
...
__host__ __device__ Real pdf_sample_phase_op::operator()(const HenyeyGreenstein &p) const {
    return c_INVFOURPI *
        (1 - p.g * p.g) / 
            (pow((1 + p.g * p.g + 2 * p.g * dot(dir_in, dir_out)), Real(3)/Real(2)));
}
