...
struct Camera {
    ...
    __host__ __device__ Camera(const Matrix4x4 &cam_to_world,
           Real fov, // in degree
...
// __device__ Ray sample_primary(const Camera &camera,
...
__device__ inline Ray sample_primary(const Camera &camera,
    ...
    Vector2 offset = sample_filter(camera.filter, Vector2{dx, dy});
