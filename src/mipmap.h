...
template <typename T>
struct ParsedMipmap {
    std::vector<ParsedImage<T>> images;
};
...
template <typename T>
inline int get_width(const ParsedMipmap<T> &mipmap) {
    assert(mipmap.images.size() > 0);
    return mipmap.images[0].width;
}

template <typename T>
inline int get_height(const ParsedMipmap<T> &mipmap) {
    assert(mipmap.images.size() > 0);
    return mipmap.images[0].height;
}

template <typename T>
inline ParsedMipmap<T> make_mipmap(const ParsedImage<T> &img) {
    ParsedMipmap<T> mipmap;
    ...
    for (int i = 1; i < num_levels; i++) {
        const ParsedImage<T> &prev_img = mipmap.images.back();
        int next_w = max(prev_img.width / 2, 1);
        int next_h = max(prev_img.height / 2, 1);
        ParsedImage<T> next_img(next_w, next_h);
...
template <typename T>
inline T lookup(const ParsedMipmap<T> &mipmap, Real u, Real v, int level) {
...
template <typename T>
__host__ __device__ inline T lookup(const Mipmap<T> &mipmap, Real u, Real v, int level) {
    assert(level >= 0 && level < (int)mipmap.len);
    // Bilinear interpolation
    // (-0.5 to match Mitsuba's coordinates)
    u = u * mipmap.images[level].width - Real(0.5);
    v = v * mipmap.images[level].height - Real(0.5);
...
template <typename T>
__host__ __device__ inline T lookup(const Mipmap<T> &mipmap, Real u, Real v, Real level) {
    if (level <= 0) {
        return lookup(mipmap, u, v, 0);
    } else if (level < Real(mipmap.len - 1)) {
        int flevel = std::clamp((int)floor(level), 0, (int)mipmap.len - 1);
        int clevel = std::clamp(flevel + 1, 0, (int)mipmap.len - 1);
        Real level_off = level - flevel;
        return lookup(mipmap, u, v, flevel) * (1 - level_off) +
               lookup(mipmap, u, v, clevel) *      level_off;
    } else {
        return lookup(mipmap, u, v, int(mipmap.len - 1));
    }
}
...
using ParsedMipmap1 = ParsedMipmap<Real>;
using ParsedMipmap3 = ParsedMipmap<Vector3>;
