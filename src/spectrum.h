...
__host__ __device__ inline Spectrum make_zero_spectrum() {
    return Vector3{0, 0, 0};
}

__host__ __device__ inline Spectrum make_const_spectrum(Real v) {
    return Vector3{v, v, v};
}

__host__ __device__ inline Spectrum fromRGB(const Vector3 &rgb) {
    return rgb;
}
...
__host__ __device__ inline Spectrum exp(const Spectrum &s) {
    return Vector3{exp(s[0]), exp(s[1]), exp(s[2])};
}

__host__ __device__ inline Real luminance(const Spectrum &s) {
    return s.x * Real(0.212671) + s.y * Real(0.715160) + s.z * Real(0.072169);
}

__host__ __device__ inline Real avg(const Spectrum &s) {
    return (s.x + s.y + s.z) / 3;
}

__host__ __device__ inline Vector3 toRGB(const Spectrum &s) {
    return s;
}
