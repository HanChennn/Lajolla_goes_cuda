PointAndNormal Parsed_sample_point_on_shape_op::operator()(const ParsedTriangleMesh &mesh) const {
    int tri_id = sample_1d_parsed(mesh.triangle_sampler, w);
...
Real Parsed_surface_area_op::operator()(const ParsedTriangleMesh &mesh) const {
    return mesh.total_area;
}

Real Parsed_pdf_point_on_shape_op::operator()(const ParsedTriangleMesh &mesh) const {
    return 1 / Parsed_surface_area_op{}(mesh);
}

void init_sampling_dist_op::operator()(ParsedTriangleMesh &mesh) const {
...
ShadingInfo Parsed_compute_shading_info_op::operator()(const ParsedTriangleMesh &mesh) const {
    ...
    if (fabs(det) > 1e-8f) {
        ...
        coordinate_system(vertex.geometric_normal, dpdu, dpdv);
    }
