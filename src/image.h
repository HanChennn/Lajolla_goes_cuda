...
struct CUImage {
    CUImage() = default;
    __device__ inline CUImage(int w, int h) : width(w), height(h) {
        data.resize(w * h);
        memset(data.data(), 0, sizeof(T) * data.size());
    }

    __device__ inline T &operator()(int x) {
        return data[x];
    }

    __device__ inline const T &operator()(int x) const {
        return data[x];
    }
    ...
    __device__ inline const T &operator()(int x, int y) const {
        return data[y * width + x];
    }
...
using CUImage1 = CUImage<Real>;
using CUImage3 = CUImage<Vector3>;
