...
ParsedImage3 aux_render(const Scene &scene) {
    int w = scene.camera.width, h = scene.camera.height;
    ParsedImage3 img(w, h);
...
ParsedImage3 path_render(const Scene &scene) {
    int w = scene.camera.width, h = scene.camera.height;
    ParsedImage3 img(w, h);
...
ParsedImage3 vol_path_render(const Scene &scene) {
    int w = scene.camera.width, h = scene.camera.height;
    ParsedImage3 img(w, h);
...
ParsedImage3 render(const Scene &scene) {
    if (scene.options.integrator == Integrator::Depth ||
        ...
        return path_render_launch(scene);
